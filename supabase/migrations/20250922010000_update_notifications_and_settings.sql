...
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS whatsapp_contact TEXT;
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.app_settings WHERE id = 1) THEN
    UPDATE public.app_settings SET whatsapp_contact = '5511999999999' WHERE id = 1 AND whatsapp_contact IS NULL;
  ELSE
